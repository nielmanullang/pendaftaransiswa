                ...
                <tbody>
                    @if (count($results) > 0)
                        @foreach ($results as $result)
                            <tr>
                            @if(Auth::user()->isAdmin())
                                <td>{{ $result['nisn'] or '' }} {{ $result['namalengkap'] or '' }}</td>
                            @endif
                                <td>{{ $result['created_at'] or '' }}</td>
                                <td>{{ $result['result'] }}/{{ $result['count'] }}={{ number_format($result['result']/$result['count']*100,2) }}</td>
                                <td>
                                    <a href="{{ route('results.show',[$result['id']]) }}" class="btn btn-xs btn-primary">
                                    @lang('quickadmin.view')</a>
                                </td>
                            </tr>
                        @endforeach
                    @else
